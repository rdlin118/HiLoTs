...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def _initialize_o3d_vis(self) -> Visualizer:
        ...
        o3d_vis.create_window(width=512, height=512, left=400, top=500)
    ...
    def _draw_pts_sem_seg(self,
        ...
        points = tensor2ndarray(points)
        if isinstance(pts_seg, PointData):
            pts_sem_seg = tensor2ndarray(pts_seg.pts_semantic_mask)
        else:
            # pts_sem_seg = tensor2ndarray(pts_seg['pts_semantic_mask'])
            pts_sem_seg = pts_seg['pts_semantic_mask']
        
        # print(f'pts_sem_seg: {pts_sem_seg}')
        # pts_sem_seg = tensor2ndarray(pts_seg.pts_semantic_mask)
        ...
        try:
            ...
            palette = np.concatenate([palette, np.array([[0,0,0]])], axis=0)
            pts_color = palette[pts_sem_seg]
    ...
    def add_datasample(self,
        ...
        # palette = np.array(palette)
        # palette[:, 0], palette[:, -1] = palette[:, -1], palette[:, 0].copy()
        ...
        if ignore_index is not None and 'gt_pts_seg' in data_sample and vis_task == 'lidar_seg':  # noqa: E501
            ...
            # print(f'')
            # print(f'PointData attributes: {data_sample.gt_pts_seg.__dir__()}')

            # print(f'eval_ann_info: {data_sample.eval_ann_info}')

            eval_ann_info = Tensor(data_sample.eval_ann_info['pts_semantic_mask']).to('cuda:0')
            # assert eval_ann_info.equal(data_sample.gt_pts_seg.pts_semantic_mask)
            ...
            if hasattr(data_sample.gt_pts_seg, 'pts_semantic_mask'):
                ...
                # pts_semantic_mask = data_sample.
                keep_index = eval_ann_info != ignore_index
        ...
        if draw_gt and data_sample is not None:
            ...
            if 'gt_pts_seg' in data_sample and vis_task == 'lidar_seg':
                ...
                if hasattr(data_sample.gt_pts_seg, 'pts_semantic_mask'):
                    ...
                    self._draw_pts_sem_seg(data_input['points'],
                                           data_sample.eval_ann_info, palette,
                                           keep_index)
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_pts_seg' in data_sample and vis_task == 'lidar_seg':
                ...
                # # try to plot diff
                # pred = data_sample.pred_pts_seg.pts_semantic_mask
                # gt = Tensor(data_sample.eval_ann_info['pts_semantic_mask']).to('cuda:0')
                # diff = (pred == gt).int()
                ...
                #     [136,217,230]
                # ]
                # data_sample.pred_pts_seg.pts_semantic_mask = diff
