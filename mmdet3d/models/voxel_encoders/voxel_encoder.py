...
class SegVFE(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_dim = 256
        self.max_tokens = 512
        self.attn = CyAttn(voxel_feat_dim=feat_compression,
                           ...
                           n_heads=1)
        self.boundary = 0.5
        self.close_linear = nn.Linear(self.embed_dim, feat_compression)
        self.dist_linear = nn.Linear(self.embed_dim, feat_compression)
    ...
    def forward(self, voxel_dict: dict) -> dict:
        ...
        batch_size = torch.max(coors[:, 0])
        attn_matrix = []
        for batch_idx in range(batch_size+1):
            voxel_idx = torch.where(voxel_coors[:, 0] == batch_idx)
            voxel = voxel_feats[voxel_idx]

            n_total = len(voxel)
            n_close = int(n_total*self.boundary)
            n_dist = n_total - n_close

            close_inds = torch.arange(n_close)
            dist_inds = torch.arange(n_close, n_total)
            assert len(close_inds) + len(dist_inds) == n_total

            sample_close = torch.tensor(np.random.choice(close_inds.cpu(), self.max_tokens))
            sample_dist = torch.tensor(np.random.choice(dist_inds.cpu(), 3*self.max_tokens))
        ...
        voxel_feats = voxel_feats + attn_matrix*self.alpha
